...
if(isset($_POST['request'])){
    $connection = mysqli_connect($host, $usernameDB, $passwordDB, $database) or die( "Unable to connect");
    if($_POST['newItemType']){
        $newItemType = $_POST['newItemType'];
        $newItemName = $_POST['newItemName'];
        echo "  newItemType = '$newItemType',  newItemName = '$newItemName' ";
        $queryAddItem = "INSERT INTO FarmItem VALUES('$newItemName', 0, '$newItemType')";
        if(mysqli_query($connection, $queryAddItem)){
            echo "reqeust submitted";
        }else {
            echo "request failed";
        }
    }
...
<form  action='manage_property.php?ID=<?php echo $_GET['ID'];?>' method='post'>
...
Current Animal:     <select name="curAnimal">
                    ...
                    <input value="delete animal" name="deleteAnimal" type="submit"><br>
Add Animal:     <select name="animal">
...
Current Crop:     <select name="curCrop">
                    ...
                    <input value="delete crop" name="deleteCrop" type="submit"><br>
Add Crop:       <select name="crop">
                    <option value="" disabled selected>Select</option>
                    <?php
                    $connection = mysqli_connect($host, $usernameDB, $passwordDB, $database) or die( "Unable to connect");
...
Item type:          <select name="newItemType">
                    <option value="" disabled selected>Select</option>
                    <?php
...
New Farm Item Name: <input type='text' name='newItemName'><br>
<input type='submit' name='request' value='Request New Item' />
<input value="Confirm Changes" name="submit" type="submit">
<input value="delete property" name="delete" type="submit">
</form>
<?php
$tmp_query = "SELECT * FROM User WHERE Username = '$owner'";
$tmp_result = mysqli_query($connection, $tmp_query);
$tmp_row = mysqli_fetch_assoc($tmp_result);
$email = $tmp_row['Email'];
echo "<a href='owner_function.php/?email=$email'>Back (Don't Save)</a><br/>";
?>
</body>
</html>